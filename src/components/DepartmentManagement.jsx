...
export function DepartmentManagement() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.work_center_id) {
      setMessage({ type: 'error', text: 'El centro de trabajo es requerido' })
      return
    }
    ...
    try {
      ...
      if (result.success) {
        setMessage({
          type: 'success',
          text: editingDepartment ? 'Departamento actualizado correctamente' : 'Departamento creado correctamente'
        })
        ...
        setMessage({ type: 'error', text: result.error || 'Error al guardar el departamento' })
      }
    } catch (err) {
      setMessage({ type: 'error', text: 'Error al guardar el departamento' })
    } finally {
      setSaving(false)
    }
  }
  ...
  const handleToggleActive = async (department) => {
    ...
    if (result.success) {
      setMessage({
        type: 'success',
        text: department.active ? 'Departamento desactivado' : 'Departamento activado'
      })
      await loadDepartments()
              ...
              <tr key={dept.id} className="hover:bg-gray-50">
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="text-sm font-medium text-gray-900">
                    {dept.work_center?.name}
                  </div>
                  <div className="text-xs text-gray-500">
                    {dept.work_center?.code}
                  </div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                  ...
                  <button
                    ...
                    className={`${
                      dept.active ? 'text-red-600 hover:text-red-900' : 'text-green-600 hover:text-green-900'
                    }`}
                  >
